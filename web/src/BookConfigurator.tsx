...
/** ③⑤/D27 书册配置器：选成品 → 填内页/插图张数 + 本数 → 实时出价（机器对客户不可见）。 */
export default function BookConfigurator({ onAdd }: { onAdd: (line: BookCartLine) => void }) {
  ...
  useEffect(() => {
    fetchBooks()
      .then(setData)
      .catch(() => {
        if (!getBooksCache()) setError('书册目录加载失败')
      })
  }, [])
  ...
  if (!data) return <p className="text-[13px] text-dim">书册目录加载中…</p>
  if (data.books.length === 0)
    return <p className="text-[13px] text-dim">暂无书册成品。</p>
