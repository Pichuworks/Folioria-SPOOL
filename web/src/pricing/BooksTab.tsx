  ...
  return (
    <div>
      ...
      <div className="flex items-baseline gap-2 border-b border-ink pb-1.5">
        <span className="bg-ink px-2 py-0.5 font-mono text-[10px] tracking-[.18em] text-paper">册子</span>
      ...
      <div className="mt-3">
        ...
        {active.length === 0 && (
          <p className="py-8 text-center text-[13px] text-dim">暂无册子成品</p>
        )}
      </div>

      <div className="pt-3 text-right font-mono text-[10px] tracking-[.1em] text-dim">
        {active.length} 册子 · {finishings.filter((f) => f.archived === 0).length} 工艺
      </div>
    </div>
  )
}
