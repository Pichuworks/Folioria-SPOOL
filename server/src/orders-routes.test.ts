 ...
 *   mode 1 黑白·单 × paper 1 打印纸70g @ A4 → 手动价 700_c；700×200/100 = 1400 分
 *   mode 6 彩图·单 × paper 6 铜版128g @ A3 → 手动价 9000_c；9000×100/100 = 9000 分
 */
...
describe('R3 下单与金额（§1.2/§1.3 唯一舍入点 + 快照）', () => {
  it('下单成功：unit_price_c 取报价、line_total 唯一舍入点、subtotal 整数加法、状态 quoted', async () => {
    ...
    expect(order.items[0]?.unit_price_c).toBe(700)
    expect(order.items[0]?.line_total).toBe(1400)
    expect(order.items[1]?.unit_price_c).toBe(9000)
    expect(order.items[1]?.line_total).toBe(9000)
    expect(order.subtotal).toBe(10400)
    expect(order.total).toBe(10400)
    expect(order.total_display).toBe('￥104.00')
    // 第二单序号递增
  ...
  it('unit_price_c 下单定格：改定价表不影响既有订单（§5）', async () => {
    ...
    db.prepare(
      `UPDATE combo_prices SET sell_c = 99900
    ...
    expect(dto.items[0]?.unit_price_c).toBe(700)
    expect(dto.items[0]?.line_total).toBe(1400)
    expect(dto.subtotal).toBe(1400)
  })
  ...
  it('数量阶梯：下单 unit_price_c 取命中档价并定格快照，改阶梯不动既有单（D38）', async () => {
    ...
    ).run(comboId, 'A4', 200, 500, null)
    const cookie = await login('a@cust.example')
    // qty 200 命中 200 档 → 单价 500；line_total = round(500×200/100)=1000（唯一舍入点）
    const order = await placeOrder(cookie, [{ mode_id: 1, paper_id: 1, size_key: 'A4', quantity: 200 }])
    expect(order.items[0]?.unit_price_c).toBe(500)
    expect(order.items[0]?.line_total).toBe(1000)
    // 改阶梯不影响既有订单（快照定格）
    db.prepare('UPDATE combo_price_tiers SET sell_c = 9900 WHERE combo_id = ? AND size_key = ?').run(comboId, 'A4')
    const after = (
      await app.inject({ method: 'GET', url: `/api/orders/${order.id}`, headers: { cookie } })
    ).json() as OrderDto
    expect(after.items[0]?.unit_price_c).toBe(500)
  })

  it('member 下单取 internal_sell_c 口径并置 is_internal=1（B1.1）', async () => {
    db.prepare(
      `UPDATE combo_prices SET internal_sell_c = 500
       WHERE size_key = 'A4' AND combo_id = (SELECT id FROM combos WHERE mode_id = 1 AND paper_id = 1)`,
    ).run()
...
describe('R6 收款与折扣（§5/C7）', () => {
  it('discount: 非整数/负数 → 422；超 subtotal → 422；合法值 total=subtotal−discount 整数减法', async () => {
    const adminCookie = await login('staff@folioria.jp')
    const cookie = await login('a@cust.example')
    const order = await placeOrder(cookie) // subtotal 10400

    const cases = [
      { payload: { discount: 1.5 }, code: 422 },
      { payload: { discount: -10 }, code: 422 },
      { payload: { discount: '10' }, code: 422 },
      { payload: { discount: 10401 }, code: 422 },
    ]
    ...
    const ok = await app.inject({
      ...
      url: `/api/orders/${order.id}/discount`,
      headers: { cookie: adminCookie },
      payload: { discount: 400 },
    })
    expect(ok.statusCode).toBe(200)
    const dto = ok.json() as OrderDto
    expect(dto.discount).toBe(400)
    expect(dto.total).toBe(10000)
    expect(dto.total_display).toBe('￥100.00')
  })

  it('折扣分摊：confirm 时 Σ(quoted_price) === total 守恒', async () => {
    const adminCookie = await login('staff@folioria.jp')
    const cookie = await login('a@cust.example')
    const order = await placeOrder(cookie) // subtotal=10400 (A4=1400, A3=9000)
    stuffFiles(order.id)
    await reviewAll(adminCookie, order, 'approved')
    ...
    // Hamilton 分摊：exact A4=53.846... A3=346.153... → base [53,346] + 余 1 给 A4
    // ⇒ quoted [1346, 8654]（每份 ≤ line_total，无负 quoted；Σ 守恒）
    expect(jobs[0]?.quoted_price).toBe(1346)
    expect(jobs[1]?.quoted_price).toBe(8654)
    expect(jobs.reduce((s, j) => s + j.quoted_price, 0)).toBe(10000)
  })
  ...
  it('D28 流水投影：押金→尾款→退款，paid_amount/status/paid_at 为 payments 投影', async () => {
    const adminCookie = await login('staff@folioria.jp')
    const cookie = await login('a@cust.example')
    const order = await placeOrder(cookie, [A4_ITEM]) // total 1400
    ...
    const deposit = await pay({ kind: 'deposit', amount: 700, method: '现金' })
  ...
  it('H2/D28: 超付、退过、kind↔符号不一致 → 422', async () => {
    ...
    expect((await pay({ kind: 'balance', amount: 1500 })).statusCode).toBe(422) // 超付
    expect((await pay({ kind: 'refund', amount: -100 })).statusCode).toBe(422) // 退过（未收先退）
    expect((await pay({ kind: 'deposit', amount: -500 })).statusCode).toBe(422) // 收款须正
    expect((await pay({ kind: 'refund', amount: 500 })).statusCode).toBe(422) // 退款须负
    // 收 1000，再收 500 会超付（1000+500>1400）→ 422；改收 400 正好结清
    expect((await pay({ kind: 'deposit', amount: 1000 })).statusCode).toBe(201)
    expect((await pay({ kind: 'balance', amount: 500 })).statusCode).toBe(422)
    const ok = await pay({ kind: 'balance', amount: 400 })
...
describe('§3.1 订单作业 done 落账回归（done 只走既有 completeJob）', () => {
  it('confirm 生成的 Job 完成后：库存扣减、quoted_price 进毛利核算', async () => {
    ...
    expect(job.quoted_price).toBe(1400)
