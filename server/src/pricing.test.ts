...
describe('§2.1 成本推导（seed 基准）', () => {
  ...
  it('ml 计价分支：mode 8 P708灌装 eff = 100 × 1000 = 100000_c → ink@A3 = 227', () => {
    ...
    expect(q?.paper_c).toBe(250)
    expect(q?.total_c).toBe(477)
  })
...
describe('§2.2 自动地板价（ceil，永不击穿 67%）', () => {
  ...
  it('性质：全部 187 组合 (auto−total)×10000 ≥ 6700×auto（整数精确，不经浮点）', () => {
    const all = listQuotable(db)
    expect(all.length).toBe(187)
    for (const q of all) {
...
describe('§2.3 报价规则（D8 防翻案：手动价生效并警示，禁止静默抬价）', () => {
  it('force OFF（默认）：黑白×70g A4 手动 7 → sell 7，below_margin', () => {
    const q = quote(db, 1, 1, 'A4')
    expect(q?.sell_c).toBe(7)
    expect(q?.auto_sell_c).toBe(19)
    expect(q?.source).toBe('manual')
    expect(q?.flag).toBe('below_margin')
  })

  it('force OFF：彩图×铜版128 A3 手动 90 → sell 90，below_margin', () => {
    const q = quote(db, 6, 6, 'A3')
    expect(q?.sell_c).toBe(90)
    expect(q?.flag).toBe('below_margin')
  })

  it('force OFF：P708原装×微喷 A3 手动 2500 < total 2523 → sell 2500，LOSS', () => {
    const q = quote(db, 7, 11, 'A3')
    expect(q?.sell_c).toBe(2500)
    expect(q?.auto_sell_c).toBe(7646)
    expect(q?.flag).toBe('LOSS')
  })

  it('force OFF：黑白×道林80 A4 无手动 → sell 22，auto', () => {
    const q = quote(db, 1, 3, 'A4')
    expect(q?.sell_c).toBe(22)
    expect(q?.auto_sell_c).toBe(22)
  ...
  it('手动价 ≥ 地板价 → 正常 manual，无警示', () => {
    db.prepare("UPDATE combo_prices SET sell_c = 30 WHERE combo_id = 1 AND size_key = 'A4'").run()
    const q = quote(db, 1, 1, 'A4')
    expect(q?.sell_c).toBe(30)
  ...
  it('内部价覆盖（B1.1）：internal_sell_c 生效，缺省回落对外逻辑', () => {
    ...
    expect(quote(db, 1, 1, 'A4', { internal: true })?.sell_c).toBe(5)
...
describe('§2.4 可选性三条件（任一不满足 → null，API 层转 404）', () => {
  ...
  it('mode 6 × paper 7 铜版157g @ A4 → 无 paper_size_cost', () => {
    expect(quote(db, 6, 7, 'A4')).toBeNull()
  })
...
describe('§2.5 全量回归基线（seed 或公式改动后变化须人工确认）', () => {
  it('187 可报价：手动 43（LOSS 4 / below_margin 25 / 正常 14）；自动 144；sell_c 全为正整数', () => {
    const all = listQuotable(db)
    expect(all.length).toBe(187)

    const manual = all.filter((q) => q.source === 'manual')
    const auto = all.filter((q) => q.source === 'auto')
    expect(manual.length).toBe(43)
    expect(auto.length).toBe(144)

    expect(manual.filter((q) => q.flag === 'LOSS').length).toBe(4)
    expect(manual.filter((q) => q.flag === 'below_margin').length).toBe(25)
    expect(manual.filter((q) => q.flag === 'manual').length).toBe(14)
...
describe('数量阶梯定价（D38 · combo_price_tiers）', () => {
  ...
  it('阶梯不进 catalog：listQuotable 仍 187（min_qty>1 不新增 qty=1 行，§2.5 基线守恒）', () => {
    setTiers([
      [100, 10, null],
      [500, 3, null],
    ])
    expect(listQuotable(db).length).toBe(187)
  })
})
