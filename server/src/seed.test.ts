...
describe('seed 导入（data/seed.json，金额已是 _c 整数，禁止换算）', () => {
  ...
  it('PRD §9 数量基线：31 尺寸 · 5 打印机 · 16 模式 · 50 纸张 · 50 采购口径 · 70 组合 · 1 耗材', () => {
    expect(count('sizes')).toBe(31)
  ...
  it('§2.5 报价基线：含开纸换算共 672，其中手动价 31，自动 641', () => {
    const quotable = listQuotable(db)
    expect(quotable).toHaveLength(672)
    expect(quotable.filter((q) => q.source === 'manual')).toHaveLength(31)
    expect(quotable.filter((q) => q.source === 'auto')).toHaveLength(641)
  })
