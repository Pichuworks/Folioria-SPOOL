...
| 用例 | 缺的条件 |
|---|---|
| mode 9 G580(max=A4) × paper 1 @ A3 | 来源纸尺寸越界 |
| 任一已有 combo 的纸张删除全部 paper_size_costs 后报价 | 无直接或可换算采购来源 |
| mode 14 × paper 10 | Combo 不存在 |

开纸换算回归：A4→A5 为 2 开，A3→A5 为 4 开，A3→B5 为 2 开，SRA3→6寸为 8 开，A3→6寸混排为 6 开。同一种纸同时存在多个可上机来源时，使用整数交叉相乘比较 `pack_price_c/(pack_count×开数)`，必须选择精确有效单张成本最低者；来源与开数只允许管理域响应展示。库存数量不得因报价换算自动变更。

### 2.5 全量回归基线

seed 全量（70 combo × 31 size，含可行开纸来源）：可报价组合 **672** 个；手动价 **31**
（LOSS **4** / below_margin **18** / manual **9**）；自动 **641**；sell_c 全部为正整数。
