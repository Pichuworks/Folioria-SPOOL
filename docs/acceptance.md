...
货币上下文：以下用例均假设 base_currency = JPY（decimal_places=0）。
`_c` = 厘（¥0.01），金额层 = 整数円。
...
|---|---|---|---|---|---|---|
| A | 6 C850彩图·单 | 6 铜版128g | A3 | 56 | 18 | 74 |
| B | 1 C850黑白·单 | 1 打印纸70g | A4 | 3 | 3 | 6 |
| C | 7 P708原装(set) | 11 微喷纸 | A3 | 2273 | 250 | 2523 |
| D | 6 C850彩图·单 | 10 铜版300g | A3 | 56 | 49 | 105 |

（C 验证 set 计价：eff=250000_c；ml 计价用 mode 8 P708灌装：eff=100×1000=100000_c）
...
|---|---|---|---|---|
| 黑白×70g A4 | 7 | 19 | **7** | below_margin（橙） |
| 彩图×铜版128 A3 | 90 | 225 | **90** | below_margin |
| P708原装×微喷 A3 | 2500 | 7646 | **2500** | LOSS（红，sell<total） |
| 黑白×道林80 A4 | 无 | 22 | 22 | auto |

force_min_margin = ON：黑白×70g A4 → sell_c=19，标记 forced。
...
| mode 6 × paper 7 铜版157g @ A4 | 无 paper_size_cost（157g 无 A4 口径） |
| mode 14 × paper 10 | Combo 不存在 |

### 2.5 全量回归基线

seed 全量（70 combo × 6 size）：可报价组合 **187** 个；手动价 **43**
（LOSS **4** / below_margin **25**）；自动 **144**；sell_c 全部为正整数。
